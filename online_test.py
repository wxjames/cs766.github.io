...
if opt.dataset == 'egogesture':
    subject_list = ['Subject{:02d}'.format(i) for i in [2, 9, 18, 19, 28, 31, 41]]
    test_paths = []
...
for path in test_paths[4:]:
        ...
        if finished_prediction == True:
            ...
            if cum_sum[best1]>opt.clf_threshold_final:
                if pre_predict == True:  
                    if best1 != prev_best1:
                        if cum_sum[best1]>opt.clf_threshold_final:  
                            results.append(((i*opt.stride_len)+opt.sample_duration_clf,best1,cum_sum[best1]))
    ...
    if opt.dataset == 'egogesture':
        ...
        # target_csv_path = os.path.join(opt.video_path, 'labels-final-revised1',
        #                         opt.whole_path.split(os.sep)[1], opt.whole_path.split(os.sep)[2],
        #                         'Group'+opt.whole_path[-1] + '.csv').replace('Subject', 'subject')
        ...
        with open(target_csv_path) as csvfile:
            ...
            for row in readCSV:
                true_classes.append(int(row[0])-1)  #int(row[0])-1
    elif opt.dataset == 'nv':
        true_classes = []
    ...
    presave.append(predicted)


    detectedframe.append(np.array(results)[:,0])
    with open("detectedframe.csv", "w") as csvfile:
        writer = csv.writer(csvfile)

        # 写入多行用writerows
        writer.writerows(detectedframe)

    prob.append(np.array(results)[:,2])
    with open("prob.csv", "w") as csvfile:
        writer = csv.writer(csvfile)

        # 写入多行用writerows
        writer.writerows(prob)

    with open("pre.csv", "w") as csvfile:
        writer = csv.writer(csvfile)

        # 写入多行用writerows
        writer.writerows(presave)

    print('True classes :\t\t',true_classes)
    actsave.append(true_classes)

    with open("act.csv", "w") as csvfile:
        writer = csv.writer(csvfile)

        # 写入多行用writerows
        writer.writerows(actsave)
